	...
	param_name(ValueBase((const char*)"skeleton"))
{
	...
	Layer_Shape::set_param("color", ValueBase(Color(0.82, 0.5, 1.0, 1.0)));
...
Layer_Skeleton::set_param(const String &param, const ValueBase &value)
{
	...
	if (param == "color")// ++ new code
		return Layer_Shape::set_param(param,value); //++ new code
...
Layer_Skeleton::get_param(const String &param)const
{
	...
	EXPORT_VALUE(color_bones); //++ new code
	...
	if (param == "color")//++ new code
		return Layer_Shape::get_param(param);//++ new code
...
Layer_Skeleton::get_param_vocab()const
{
	...
	ret.push_back(ParamDesc("color") //++ new code
		.set_local_name(_("Color bones"))//++ new code
	);//++ new code
